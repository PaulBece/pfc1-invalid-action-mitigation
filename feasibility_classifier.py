...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PPO agent with Feasibility Classification')
    # Common arguments
    ...
    # Feasibility Classification specific argument
    parser.add_argument('--cls-coef', type=float, default=10.0,
                        help="coefficient/strength of the auxiliary feasibility classification loss (lambda)")
...
class Agent(nn.Module):
    def __init__(self, frames=4):
        ...
        # Auxiliary Classification Head for Feasibility Prediction
        self.feasibility_classifier = layer_init(nn.Linear(256, envs.action_space.nvec.sum()), std=1.0)
    ...
    def predict_feasibility(self, x):
        # Outputs unnormalized logits for the auxiliary validity tasks
        return self.feasibility_classifier(self.forward(x))
...
for update in range(starting_update, num_updates+1):
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            # Feasibility Classification Loss (BCE Loss with Logits)
            pred_feasibility_logits = agent.predict_feasibility(b_obs[minibatch_ind])
            # Ground-truth validity target (1.0 for valid action slot, 0.0 for invalid)
            gt_feasibility_targets = b_invalid_action_masks[minibatch_ind].float()
            cls_loss = F.binary_cross_entropy_with_logits(pred_feasibility_logits, gt_feasibility_targets)

            # Calculate Feasibility Prediction Accuracy (for Dashboard Tracking)
            with torch.no_grad():
                pred_binary = (torch.sigmoid(pred_feasibility_logits) > 0.5).float()
                cls_accuracy = (pred_binary == gt_feasibility_targets).float().mean()

            # Aggregated Joint Loss Objective (L_total = L_PPO + lambda * L_cls)
            loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + args.cls_coef * cls_loss
    ...
    # Feasibility Classifier Logging
    writer.add_scalar("losses/classification_loss", cls_loss.item(), global_step)
    writer.add_scalar("charts/classification_accuracy", cls_accuracy.item(), global_step)
    
    if args.kle_stop or args.kle_rollback:
        writer.add_scalar("debug/pg_stop_iter", i_epoch_pi, global_step)
    writer.add_scalar("charts/sps", int(global_step / (time.time() - start_time)), global_step)
    print(f"SPS: {int(global_step / (time.time() - start_time))} | Cls Accuracy: {cls_accuracy.item():.4f} | Cls Loss: {cls_loss.item():.4f}")
